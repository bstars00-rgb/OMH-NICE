...
function viewDashboard(){
  ...
  const table = list.length ? `<div class="panel"><h3 style="display:flex;align-items:center;justify-content:space-between;gap:12px">${T('summaryTitle')} <button class="btn sm" data-act="copysummary:active">${T('copyFormatted')}</button></h3>
  ...
  const completedPanel = completed.length ? `<div class="panel" style="border-color:#c6ebd3"><h3 style="background:#eaf7ef;display:flex;align-items:center;justify-content:space-between;gap:12px"><span>✅ ${LANG==='en'?'Approved / Completed (CEO final approval)':'승인 완료 (대표이사 최종 승인)'} <span class="pill">${completed.length}</span></span> <button class="btn sm" data-act="copysummary:completed">${T('copyFormatted')}</button></h3>
...
function copySummary(section){
  const comps = sectionCompanies(section);
  ...
  const okMsg = LANG==='en'
    ? (comps.length+' '+sectionLabel(section)+' risk summaries copied with formatting. Paste into email · Word · Excel.')
    : ('['+sectionLabel(section)+'] '+comps.length+'개 업체 리스크 요약이 서식 포함으로 복사되었습니다. 메일·Word·Excel에 붙여넣기 하세요.');
