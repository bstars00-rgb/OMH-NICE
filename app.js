...
function seed(){
  ...
  const c7 = mk({ id:'C007', name:B('돌하루팡 (주식회사 제주페이)','Dolharupang (Jeju Pay Inc.)'), country:B('한국(제주)','Korea (Jeju)'), businessType:B('B2C','B2C'),
    ...
    website:B('www.dolharupang.com','www.dolharupang.com'), bizRegNo:B('사업자등록증 제출·확인(주식회사 제주페이)','Business license submitted·verified (Jeju Pay Inc.)'), foundedYear:'', representative:B('사업자등록증 상 대표자 확인','Representative per business license'),
    contact:'Global Sales', email:'', deposit:30000, settlementDays:14, currency:'KRW', creditRequired:'N', monthlyGMV:0, salesRegion:B('한국(KR)','Korea (KR)'), products:B('B2C 예약 플랫폼(제주·국내 호텔)','B2C booking platform (Jeju·domestic hotels)'), apiIntegration:'Y', manualBooking:'N', cancelNoshowRisk:'low',
    scores:[3,4,4,3,4,3,3,3,4,4,4,4,3,3], status:'review',
    docs:{bizLicense:'submitted',profile:'notSubmitted',bank:'notSubmitted',contract:'notSubmitted',settleAgree:'notSubmitted',refs:'notSubmitted',financials:'notSubmitted',repId:'notSubmitted'},
    ...
    documents:[{name:B('사업자등록증(주식회사 제주페이)','Business license (Jeju Pay Inc.)'),file:'제주페이_사업자등록증_25.09.10 (5).pdf'}],
    notes:{expect:B('제주 기반 B2C 예약 플랫폼 "돌하루팡"(주식회사 제주페이). 사업자등록증 확인(한국 법인). Direct API 연동, 소스마켓 KR 대상 B2C 채널 → 오마이호텔 제주·국내 호텔 재고의 신규 국내 B2C 유통 채널 가능. Deposit USD 30,000(신규 업체 치 견고), 격주(2주) 정산·정산통화 KRW.','Jeju-based B2C booking platform "Dolharupang" (Jeju Pay Inc.). Business license verified (Korean entity). Direct API, KR source-market B2C channel → potential new domestic B2C distribution for Ohmyhotel Jeju·domestic hotel inventory. Deposit USD 30,000 (solid for a new partner), biweekly settlement in KRW.'),
      check:B('신규 업체로 거래 실적·예상 월거래액(GMV) 미확인 → 볼륨·매출 기여 산정 불가(확인필요). 회사소개서·재무제표·은행정보·계약서·정산합의서 미제출(사업자등록증만 확보). Deposit는 USD, 정산통화는 KRW 혼용 → 환리스크·정산 처리 방식 확인. 제주 B2C 니치라 초기 물량·경쟁력 검증 필요. 계약 승인일(Approved date) 미정.','New partner — no trading history·expected monthly GMV (TBC) → volume·revenue contribution not yet quantifiable. Company profile·financials·bank·contract·settlement agreement not submitted (only business license). Deposit in USD but settlement in KRW → confirm FX risk·settlement handling. Jeju B2C niche → verify initial volume·competitiveness. Approval date pending.'),
      opinion:B('한국 법인·사업자등록 확인·Direct API·견고한 Deposit로 기본 요건은 양호하나, 신규라 실적·예상 물량 미검증 → 점수 69점대 C(추가확인, B 근접). 예상 월거래액·초기 실적·회사소개서 확보 및 초기 3개월 모니터링 조건으로 조건부 승인 검토 가능.','Korean entity·verified license·Direct API·solid deposit make the basics sound, but as a new partner track record·expected volume are unverified → ~69 score, C (additional check, near B). Conditional approval feasible once expected GMV·initial performance·company profile are secured, with 3-month monitoring.'),
      comment:B('신규 제주 B2C 채널. 사업자·Deposit·API 양호하나 물량 미확인으로 추가확인(3/5). 예상 GMV·초기 실적 확인 후 승인 권고.','New Jeju B2C channel. Entity·deposit·API are good, but volume unconfirmed → additional check (3/5). Recommend approval after confirming expected GMV·initial performance.')},
