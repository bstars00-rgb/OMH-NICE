...
const I18N = {
  ko:{
    ...
    summaryTitle:'업체별 리스크 요약', copyFormatted:'📋 서식 복사', copyScopeHint:'(선택 시 선택분만 · 미선택 시 전체 복사)', selectAll:'전체 선택',
  ...
  en:{
    ...
    summaryTitle:'Partner risk summary', copyFormatted:'📋 Copy formatted', copyScopeHint:'(checked → selected only · none → all)', selectAll:'Select all',
...
function compute(c){
  ...
  const uncoveredCredit = c.creditRequired==='Y' && dep===0 && c.creditApproved!=='Y';
...
function viewDashboard(){
  ...
  const rows = list.map(({c,r})=>`
    <tr data-open="${c.id}">
      <td class="ctr selcell no-print"><input type="checkbox" class="rowsel" data-selid="${c.id}"></td>
      ...
      <td class="ctr">${badge(r.finalGrade)}<div style="margin-top:5px"><span class="status-chip">${esc(defLabel(STATUS_DEFS,c.status)||'-')}</span></div></td>
  ...
  const table = list.length ? `<div class="panel"><h3 style="display:flex;align-items:center;justify-content:space-between;gap:12px"><span>${T('summaryTitle')} <span class="hint" style="font-weight:400">${T('copyScopeHint')}</span></span> <button class="btn sm" data-act="copysummary:active">${T('copyFormatted')}</button></h3>
    <div class="table-wrap"><table><thead><tr>
      <th class="ctr no-print"><input type="checkbox" data-selall title="${T('selectAll')}"></th>
  ...
  const completedPanel = completed.length ? `<div class="panel" style="border-color:#c6ebd3"><h3 style="background:#eaf7ef;display:flex;align-items:center;justify-content:space-between;gap:12px"><span>✅ ${LANG==='en'?'Approved / Completed (CEO final approval)':'승인 완료 (대표이사 최종 승인)'} <span class="pill">${completed.length}</span> <span class="hint" style="font-weight:400">${T('copyScopeHint')}</span></span> <button class="btn sm" data-act="copysummary:completed">${T('copyFormatted')}</button></h3>
    <div class="table-wrap"><table><thead><tr><th class="ctr no-print"><input type="checkbox" data-selall title="${T('selectAll')}"></th><th>${T('thCompany')}</th><th class="ctr">${T('thCountry')}</th><th class="num">${T('thDeposit')}</th><th class="ctr">${T('thSettle')}</th><th class="ctr">${T('thFinal')}</th><th>${LANG==='en'?'Note':'비고'}</th></tr></thead><tbody>${compRows}</tbody></table></div></div>` : '';
  ...
  const benchmarkPanel = benchmark.length ? `<div class="panel" style="border-color:#d5deea"><h3 style="background:#f4f7fb;display:flex;align-items:center;justify-content:space-between;gap:12px"><span>🏛️ ${LANG==='en'?'Reference — self-assessment (benchmark, not a trading partner)':'참고용 — 자체평가(벤치마크 · 거래 대상 아님)'} <span class="pill">${benchmark.length}</span></span> <button class="btn sm" data-act="copysummary:benchmark">${T('copyFormatted')}</button></h3>
    <div class="table-wrap"><table><thead><tr><th class="ctr no-print"><input type="checkbox" data-selall title="${T('selectAll')}"></th><th>${T('thCompany')}</th><th class="ctr">${T('thCountry')}</th><th class="num">${T('thDeposit')}</th><th class="ctr">${T('thSettle')}</th><th class="num">${T('thWeighted')}</th><th class="ctr">${T('thFinal')}</th><th>${LANG==='en'?'Note':'비고'}</th></tr></thead><tbody>${benchRows}</tbody></table></div></div>` : '';
...
function act(a){
  ...
  else if(a==='copysummary' || a.indexOf('copysummary:')===0){ copySummary(a.split(':')[1]||'active'); }
}

/* content-level delegation (inputs, tabs, row open) */
const content = document.getElementById('content');
content.addEventListener('click', e=>{
  const selall = e.target.closest('input[data-selall]');
  if(selall){ const tbl=selall.closest('table'); if(tbl) tbl.querySelectorAll('.rowsel').forEach(cb=>cb.checked=selall.checked); return; }
  if(e.target.closest('.selcell')){ return; } // 행 체크박스: 상세로 이동하지 않음
...
function sectionCompanies(section){
  const all = DATA.companies;
  if(section==='completed') return all.filter(c=>c.status==='approved' && !c.benchmark);
  if(section==='benchmark') return all.filter(c=>c.benchmark);
  if(section==='active') return all.filter(c=>!c.benchmark && c.status!=='approved');
  return all; // 'all' or undefined → 전체
}
function sectionLabel(section){
  if(LANG==='en') return section==='completed'?'approved/completed':section==='benchmark'?'reference (self-assessment)':section==='active'?'in-review partner':'partner';
  return section==='completed'?'승인 완료':section==='benchmark'?'참고용(자체평가)':section==='active'?'평가 진행 업체':'업체';
}
function copySummary(section){
  const secComps = sectionCompanies(section);
  const secIds = new Set(secComps.map(c=>c.id));
  const checkedInSec = Array.from(document.querySelectorAll('.rowsel:checked')).map(el=>el.dataset.selid).filter(id=>secIds.has(id));
  const comps = checkedInSec.length ? secComps.filter(c=>checkedInSec.includes(c.id)) : secComps;
  const partial = checkedInSec.length>0;
  ...
  html += '</tbody></table>';
  if(!comps.length){ alert(LANG==='en'?'No rows to copy in this section.':'이 섹션에 복사할 항목이 없습니다.'); return; }
  const scope = partial ? (LANG==='en'?'selected ':'선택 ') : (LANG==='en'?'all ':'전체 ');
  const okMsg = LANG==='en'
    ? (scope+comps.length+' of '+sectionLabel(section)+' risk summaries copied with formatting. Paste into email · Word · Excel.')
    : ('['+sectionLabel(section)+'] '+scope+comps.length+'개 업체 리스크 요약이 서식 포함으로 복사되었습니다. 메일·Word·Excel에 붙여넣기 하세요.');
...
function seed(){
  ...
  const c3 = mk({ id:'C003', name:'Wingpulse (WINGSPULSE TECH)', country:B('중국(홍콩·선전 거점)','China (HK·Shenzhen bases)'), businessType:'B2B/TMC',
    market:B('호텔 직계약+LCC 항공 (Travel Tech)','Hotel direct-contract + LCC air (Travel Tech)'), customerType:B('B2B·TMC·서브에이전트(OTA는 Elong·Qunar)','B2B·TMC·sub-agent (OTA: Elong·Qunar)'),
    website:'http://wingspulse.com / info@wingspulse.com', bizRegNo:'51588485 (HK BR)', foundedYear:2025, representative:B('HE PENG(贺鹏), 단독이사','HE PENG (贺鹏), sole director'),
    contact:'WingsPulse', email:'info@wingspulse.com', deposit:20000, monthlyGMV:40000, salesRegion:B('한국·동남아·중국(HK·대만·마카오)','Korea·SE Asia·China (HK·Taiwan·Macau)'), products:B('한국 등 역내 호텔 260+ 직계약 · LCC 항공 15+사','Korea+regional 260+ direct hotels · 15+ LCC airlines'),
    scores:[3,4,4,4,3,4,4,3,3,4,3,3,4,4], status:'review',
    ...
    notes:{expect:B('AI·시맨틱 태깅 기반 Travel Tech(홍콩·선전 거점), 밀리초 API·실시간 재고. [호텔] 한국·동남아·중국(홍콩·대만·마카오) 260+ 직계약 + 현지 DMC. [항공] Ryanair·Southwest·easyJet 등 15+ LCC/지역항공사 직계약, 항공권 집계·발권. Top5 목적지 태국·말레이·베트남·한국·일본, 주요 고객 중국시장. 일 생산량 일본 10·한국 15·베트남 12 RN. 판매채널 B2B·TMC·서브에이전트(OTA는 Elong·Qunar만 → BRG 리스크 낮음). 크로스셀 파트너십 관심, 초기 당사 인벤토리 우선 구매 의향.','AI/semantic-tagging Travel Tech (HK·Shenzhen), millisecond API·real-time inventory. [Hotels] 260+ direct contracts across Korea·SE Asia·China (HK·Taiwan·Macau) + local DMCs. [Air] 15+ LCC/regional airlines direct (Ryanair·Southwest·easyJet), fare aggregation·ticketing. Top-5 destinations Thailand·Malaysia·Vietnam·Korea·Japan; main customers China market. Daily production JP 10·KR 15·VN 12 RN. Channels B2B·TMC·sub-agent (OTA only Elong·Qunar → low BRG risk). Interested in cross-sell partnership, willing to buy our inventory first initially.'),
      check:B('★회사 제공 일일 생산량(≈37 RN/day, 한국 15 포함)·15 LCC·260 직계약은 실제 영업을 시사하나, 제출된 감사 재무제표(2026.3)는 무영업·매출 0으로 상충 → 실거래 증빙(은행 거래내역·예약 데이터)로 검증 필요. 은행정보·계약서 미제출, 자산 대부분 이사·주주 대여금 해명 필요. 정산 격주·USD(결제기한 7일).','★Company-reported daily production (~37 RN/day incl. Korea 15)·15 LCC·260 direct contracts suggest real operations, but the submitted audited financials (Mar 2026) show inactive/zero revenue — a conflict → verify with real-transaction evidence (bank statements·booking data). Bank details·contract not submitted; assets mostly director/shareholder loans need explanation. Biweekly·USD (7-day payment term).'),
      opinion:B('실제 생산량(한국 15 RN/day 등)·15 LCC·260 직계약·당사 인벤토리 우선구매 의향으로 C→B(조건부 승인). 조건: ①일일 RN·매출 실거래 증빙으로 감사 무영업 상충 해소 ②은행정보·계약서 ③초기 소액 파일럿·선입금/낮은 한도·3개월 모니터링. 증빙 검증 전까지 안전장치 유지.','Real production (e.g., Korea 15 RN/day)·15 LCC·260 direct contracts·intent to buy our inventory first → C→B (conditional approval). Conditions: (1) verify daily RN·revenue with real-transaction proof to resolve the inactive-audit conflict, (2) bank details·contract, (3) small initial pilot·prepay/low limit·3-month monitoring. Keep safeguards until verified.'),
      comment:B('실거래 생산량·항공·크로스셀 의향 확인으로 상향(4/5). 단 감사상 무영업과 상충 → 실거래 증빙 검증 조건부.','Raised on evidence of real production·air·cross-sell intent (4/5). But it conflicts with the inactive audit → conditional on verifying real transactions.')},
    history:[{stage:'sales',reviewer:'Global OPs',decision:'proceed',comment:B('TMC/커넥티비티 기술 파트너 후보','TMC/connectivity tech partner candidate'),date:'2026-06-27'},
      ...
      {stage:'scm',reviewer:'Global OPs',decision:'proceed',comment:B('업체 업데이트 반영: 일 생산 JP10·KR15·VN12 RN, 15+ LCC(Ryanair 등), 260 직계약, 당사 인벤토리 우선구매 의향 → C→B(조건부). 실거래 증빙(은행내역·예약데이터)로 감사 무영업 상충 해소 조건.','Update: daily RN JP10·KR15·VN12, 15+ LCC (Ryanair etc.), 260 direct contracts, intent to buy our inventory first → C→B (conditional). Condition: verify real transactions (bank·booking data) to resolve the inactive-audit conflict.'),date:'2026-07-14'},
      {stage:'finance',reviewer:'Global OPs',decision:'conditional',comment:B('거래조건 합의: Deposit $10,000→$20,000·2주(14일)·USD. 점수 70.4(B)라 정책 권장 디포짓은 $30,000 → $20,000은 $10k 미달분에 대해 대표 예외 승인 필요.','Terms agreed: Deposit $10,000→$20,000·biweekly (14d)·USD. Score 70.4 (B), so policy-recommended deposit is $30,000 → $20,000 is $10k short and needs CEO exception approval.'),date:'2026-08-13'}] });
  ...
  const c8 = mk({ id:'C008', name:B('Asiarooms (주식회사 제이알홀딩스)','Asiarooms (JR Holdings Inc.)'), country:B('한국(본사)·베트남 사무소','Korea (HQ)·Vietnam office'), businessType:B('B2B 호텔 공급사(Wholesaler)','B2B hotel supplier (wholesaler)'),
    ...
    contact:'Asiarooms Sales', email:'sales2@asiarooms.ai', deposit:10000, settlementDays:14, currency:'USD', creditRequired:'N', monthlyGMV:0, salesRegion:B('베트남·일본·한국·동남아','Vietnam·Japan·Korea·SE Asia'), products:B('호텔 인벤토리 공급(베트남 500+ 직계약·일본 독점 500+·한국 300+·SEA 20,000+)','Hotel inventory supply (Vietnam 500+ direct·Japan 500+ exclusive·Korea 300+·SEA 20,000+)'), apiIntegration:'Y', manualBooking:'N', cancelNoshowRisk:'low',
    scores:[3,4,3,4,4,4,3,3,3,5,4,4,4,3], status:'review',
    ...
    notes:{expect:B('아시아 B2B 호텔 유통·공급 플랫폼 Asiarooms(운영 주식회사 제이알홀딩스, 대표 정길원). 2023 설립·2025 정식 운영, 한국 본사(경기 부천)·베트남 사무소·24/7. 공급 인벤토리: 베트남 500+ 직계약(연말 1,000 목표, 정적·동적 요금, 3-5성·풀빌라·리조트), 일본 독점 500+(OTA 미노출 hidden), 한국 300+(서울·부산·제주·경주·인천), SEA 20,000+. 룸매핑·콘텐츠 정제 강점, API Pull/Push/Switch. 오마이호텔에 인벤토리 공급 희망(상호 크로스셀 가능).','Asia B2B hotel distribution·supply platform Asiarooms (operator JR Holdings Inc., rep. Jung Gil-won). Founded 2023, full ops since 2025; Korea HQ (Bucheon)·Vietnam office·24/7. Supply inventory: Vietnam 500+ direct (target 1,000 by year-end; static·dynamic; 3-5-star·pool villas·resorts), Japan 500+ exclusive (hidden, not on OTAs), Korea 300+ (Seoul·Busan·Jeju·Gyeongju·Incheon), SEA 20,000+. Strong room-mapping·content, API Pull/Push/Switch. Wants to supply inventory to Ohmyhotel (cross-sell possible).'),
      check:B('★[대표이사 방침] 판매 전용(리셀) 업체 지양 → Asiarooms가 베트남 500+·일본 독점을 실제 직접 구매/직계약하는지(allotment·계약서) 검증 필수(단순 리셀이면 지양 대상). 공급사(정산 방향 반대) — Asiarooms가 Deposit $10,000 요청(주체·방향 확인 필요; 신설 공급사에 OMH 자금 선노출 주의). 2023 설립·트랙레코드 짧음. 정산·수수료(net/commission)·환불·오버부킹 정책 및 재무·은행 실사 미완.','★[CEO directive] Avoid sell-only (reseller) partners → must verify Asiarooms actually directly purchases/contracts the Vietnam 500+·Japan-exclusive inventory (allotment·contracts); if merely reselling, it is a partner to avoid. Supplier (settlement reversed) — Asiarooms requests a USD 10,000 deposit (confirm party·direction; caution on pre-exposing OMH funds to a new supplier). Founded 2023, short track record. Settlement·commission (net/commission)·refund·overbooking policy and financial·bank due diligence pending.'),
      opinion:B('유니크 인벤토리·기술은 매력적이나, 그 가치는 "실제 직접 구매(직계약)" 검증에 달려 있음 → 추가 확인(C). 대표이사 방침(판매 전용 지양)에 따라 직접 구매·allotment 증빙이 승인 전제. 검증되면 B(조건부 승인)로 상향 가능. 파일럿으로 요금·재고·직계약 실체 동시 확인 권장.','Unique inventory·tech are attractive, but their value hinges on verifying real direct purchasing (direct contracts) → needs review (C). Per the CEO directive (avoid sell-only), proof of direct purchasing·allotment is a precondition for approval. If verified, upgradeable to B (conditional). Recommend a pilot to confirm rates·inventory·direct-contract reality together.'),
      comment:B('인벤토리·기술은 좋으나 "직접 구매(직계약)" 실체 미검증 → 대표 방침상 신중(3/5). 리셀 여부·allotment 확인 후 판단.','Inventory·tech look good, but real direct purchasing (direct contracts) is unverified → cautious per CEO directive (3/5). Decide after confirming resale-vs-direct and allotment.')},
    history:[{stage:'sales',reviewer:'Global Sales',decision:'proceed',comment:B('신규 공급사 등록 — Asiarooms(JR Holdings Inc., 사업자 268-88-03052, 대표 정길원). 베트남 500+ 직계약·일본 독점·한국 300+·SEA 20,000+ 인벤토리 공급 희망, API Pull/Push/Switch. 정산/요금/실사 협의 예정.','New supplier intake — Asiarooms (JR Holdings Inc., biz-reg 268-88-03052, rep. Jung Gil-won). Wants to supply Vietnam 500+ direct·Japan exclusive·Korea 300+·SEA 20,000+ inventory, API Pull/Push/Switch. Settlement/rate/DD to be discussed.'),date:'2026-07-17'},
      {stage:'scm',reviewer:'Global OPs',decision:'hold',comment:B('대표이사 방침 반영: 판매 전용(리셀) 업체 지양 → Asiarooms 직접 구매·직계약 실체(allotment·계약서) 검증 필요. Deposit $10,000 요청(방향 확인). 검증 전까지 C(추가확인).','Per CEO directive: avoid sell-only (resellers) → verify Asiarooms real direct purchasing·contracts (allotment·contracts). Requests USD 10,000 deposit (confirm direction). Held at C pending verification.'),date:'2026-07-17'}] });
  const c9 = mk({ id:'C009', name:B('梓如商旅 (Ziru Business Travel Group)','Ziru Business Travel Group (梓如商旅)'), country:B('중국(난징)','China (Nanjing)'), businessType:'B2B/TMC',
    ...
    notes:{expect:B('梓如商务旅行服务集团(Ziru, 2007 설립·난징·자본 8,705만 CNY·법정대표 廖任熙). 중국 유일 국자배경 B2B 전용 TMC — 국央기업·상장사·정부(24개 성급 인대) 差旅 관리. 자체 SW 60+저작권·ISO27001/9001·CMMI·공안부 3급 등보, 남경·성도 R&D·18개 계열사. 항공 130만+ 대리권(GDS 직연)·호텔 160만+/자체협의 5만+·기차·용차·ChatBI 스마트리포트(긍정 언론). OA/ERP/费控 연동. → OMH 호텔(특히 한국·해외)의 대규모 기업 差旅 수요 채널·크로스셀 가능.','Ziru Business Travel Service Group (est. 2007, Nanjing, capital ~CNY 87.05M, legal rep Liao Renxi). China unique state-owned-background B2B-only TMC — manages business travel for SOEs·listed firms·government (24 provincial congress groups). 60+ software copyrights·ISO27001/9001·CMMI·MPS level-3, Nanjing·Chengdu R&D·18 subsidiaries. Air 1.3M+ agency rights (GDS direct)·hotels 1.6M+/50k+ self-signed·rail·car·ChatBI reporting (positive press). OA/ERP/expense-control integration. Potential large corporate-travel demand channel·cross-sell for OMH hotels (esp. Korea·overseas).'),
      check:B('대형·국자배경 TMC로 실체·신뢰도 최상위이나 ①OMH向 실질 거래량 미검증(자체 호텔 공급 대규모 보유 → 크로스셀 실익 불명) ②재무제표·은행정보 미제출(자본금은 license상 확인) ③정산 통화(USD)·수수료·중국-한국 크로스보더 정산방식 협의 ④주 고객이 중국 국내 差旅라 OMH 한국·해외 인벤토리 정합 파일럿 필요.','Top-tier entity·credibility (large state-owned-background TMC), but (1) OMH-bound real volume unverified (holds large own hotel supply → cross-sell upside unproven), (2) financials·bank not submitted (capital confirmed on license), (3) settlement currency (USD)·commission·China-Korea cross-border settlement to agree, (4) mainly China-domestic travel → pilot needed for OMH Korea·overseas inventory fit.'),
      opinion:B('신뢰도·기술·규모는 A급이나 OMH 상업적 실익(거래량) 미검증 → B(조건부 승인, A 근접). 조건: 파일럿으로 OMH 인벤토리 실거래·정산 검증, 재무·은행 실사, 크로스보더 정산 합의. 실적 확인 시 A 상향 가능. TMC(실수요 채널)로 대표 방침(판매전용 지양)에 부합.','Credibility·tech·scale are A-grade, but OMH commercial upside (volume) is unverified → B (conditional approval, near A). Conditions: pilot to verify real OMH-inventory transactions·settlement, financial·bank DD, cross-border settlement agreement. Upgradeable to A once proven. As a TMC (real demand channel) it fits the CEO directive (avoid sell-only).'),
      comment:B('국자배경·2007·ISO/CMMI로 최상위 신뢰도(5/5). OMH 실거래·크로스보더 정산 파일럿 후 승인 권고.','State-owned background·2007·ISO/CMMI give top credibility (5/5). Recommend approval after an OMH real-transaction·cross-border settlement pilot.')},
    history:[{stage:'sales',reviewer:'Global Sales',decision:'proceed',comment:B('신규 TMC 등록 — 梓如商旅(Ziru, 신용코드 913201136673802326, 2007 설립, 대표 廖任熙). 국자배경 B2B TMC, 국유기업·정부 고객. 2주 정산·USD·Deposit $10k. OMH 호텔(한국·해외) 差旅 수요 채널·크로스셀 검토.','New TMC intake — Ziru (credit code 913201136673802326, est. 2007, rep Liao Renxi). State-owned-background B2B TMC, SOE·government clients. Biweekly·USD·USD 10k deposit. Reviewing as demand channel for OMH hotels (Korea·overseas)·cross-sell.'),date:'2026-07-17'}] });
  const c10 = mk({ id:'C010', name:B('滴滴企业版 (DiDi Enterprise · DIDI TMC)','DiDi Enterprise (滴滴企业版)'), country:B('중국(천진 분공사·베이징 본사)','China (Tianjin branch·Beijing HQ)'), businessType:'B2B/TMC',
    ...
    notes:{expect:B('滴滴企业版(Didi Enterprise) — 중국 최대급 모빌리티 Didi(小桔) 계열의 기업 差旅 플랫폼. 계약 주체는 北京小桔国际旅행社 天津分公司(신용코드 91120116MA82669937, 2023.5.25 설립, 负责人 郭起超). 대규모 중국 기업 고객 대상 원스톱 差旅·비용환급(费控) 관리. 해외 호텔: Agoda·Expedia 등 OTA + Hotelbeds·Webbeds·道旅 도매상 접입(200+국·116만+호텔), GDS 직연 호텔그룹(万豪·洲际·希尔顿·雅高), 협의호텔 托管, VCC 가상카드로 境内 RMB 결산·全球 지급, 7×24 중영문 CS. → OMH 한국·아시아 호텔의 대규모 기업 差旅 수요 채널·크로스셀 가능(단, 이미 OTA/도매상 리셀 소싱 중이라 OMH 요금 경쟁력이 관건).','DiDi Enterprise Solutions — corporate business-travel platform of Didi (Xiaoju) group. Contracting entity: Beijing Xiaoju International Travel Agency, Tianjin Branch (credit code 91120116MA82669937, est. 2023-05-25, branch head Guo Qichao). One-stop business-travel·expense-control for large Chinese enterprises. Overseas hotels: OTAs (Agoda·Expedia) + wholesalers (Hotelbeds·Webbeds·Didatravel), 200+ countries·1.16M+ hotels, GDS-direct hotel groups (Marriott·IHG·Hilton·Accor), agreement-hotel hosting, VCC virtual cards for onshore-RMB-settlement·global-pay, 7x24 CN/EN support. Potential large corporate-travel demand channel·cross-sell for OMH Korea·Asia hotels (but already resells from OTAs/wholesalers, so OMH rate competitiveness is key).'),
      check:B('대형 Didi 브랜드·기업 差旅 수요는 강점이나 ①계약 주체가 2023 설립 분공사(天津分公司) — 본사(北京小桔)·모기업(Didi) 보증·정산 주체 확인 필요 ②Didi 그룹 과거 규제 이력(데이터·상장폐지, 現 해소)은 참고 ③해외 호텔을 이미 Agoda·Expedia·Hotelbeds·道旅에서 리셀 소싱 → OMH 요금 경쟁력·차별성 검증 필요 ④크로스보더 정산(USD/RMB·VCC)·수수료·재무 실사 미완.','Large Didi brand·enterprise demand are strengths, but (1) contracting entity is a 2023 branch (Tianjin) — confirm head office (Beijing Xiaoju)·Didi guarantee·settlement party, (2) Didi group past regulatory history (data·delisting, now resolved) noted, (3) overseas hotels already resold from Agoda·Expedia·Hotelbeds·Didatravel → verify OMH rate competitiveness·differentiation, (4) cross-border settlement (USD/RMB·VCC)·commission·financial DD pending.'),
      opinion:B('대규모 기업 差旅 수요 채널로 잠재력 큼 → B(조건부 승인). 조건: 요금 경쟁력 파일럿(기존 Agoda/道旅 대비), 계약·정산 주체(분공사 vs 본사) 및 크로스보더 정산 합의, 재무·모기업 보증 확인. TMC(실수요)로 대표 방침(판매전용 지양)엔 부합. 요금 경쟁력·실거래 확인 시 상향 가능.','Strong potential as a large corporate-travel demand channel → B (conditional approval). Conditions: rate-competitiveness pilot (vs existing Agoda/Didatravel), agree contracting·settlement party (branch vs head office) and cross-border settlement, confirm financials·parent guarantee. As a TMC (real demand) it fits the CEO directive (avoid sell-only). Upgradeable once rates·real transactions are verified.'),
      comment:B('Didi 브랜드·기업 수요는 강력(4/5). 단 계약주체(2023 분공사)·리셀 소싱 대비 OMH 요금 경쟁력 확인 후 승인 권고.','Didi brand·enterprise demand are powerful (4/5). Recommend approval after confirming contracting party (2023 branch) and OMH rate competitiveness vs reseller sourcing.')},
    history:[{stage:'sales',reviewer:'Global Sales',decision:'proceed',comment:B('신규 TMC — 滴滴企业版(北京小桔国际旅행社 天津分公司, 신용코드 91120116MA82669937, 2023, 负责人 郭起超). 기업 差旅 해외호텔 채널. 2주·USD·Deposit $10k. OMH 한국·아시아 호텔 공급 검토. 계약·정산 주체·요금 경쟁력 확인 필요.','New TMC — DiDi Enterprise (Beijing Xiaoju Intl Travel, Tianjin Branch, credit code 91120116MA82669937, 2023, head Guo Qichao). Corporate overseas-hotel channel. Biweekly·USD·USD 10k deposit. Reviewing OMH Korea·Asia hotel supply; confirm contracting·settlement party·rate competitiveness.'),date:'2026-07-17'}] });
  const c11 = mk({ id:'C011', name:B('Travelata (RU · UAE 계약법인)','Travelata (RU · UAE contracting entity)'), country:B('러시아(모스크바) · UAE 계약법인','Russia (Moscow) · UAE contracting entity'), businessType:'OTA(B2C)',
    ...
    notes:{expect:B('러시아 대형 OTA Travelata — 운영사 Travel Technology LLC(2011, 모스크바). 패키지 비교·예약 B2C. 러시아 아웃바운드를 베트남(1)·태국·한국·싱가포르로 → OMH 아시아 호텔의 신규 러시아 채널. [계약 주체] UAE 법인 Vivatech Platform L.L.C-FZ(두바이 Meydan Free Zone, License 2643677.01, TRN 105369350100001, 2026.1 설립, 이사·100% 단독주주 Alexey Zaretsky·이스라엘 국적, Emirates Islamic Bank USD 계좌 보유). Direct API(Travelata 개발), 초기 5~10건/일.','Large Russian OTA Travelata — operator Travel Technology LLC (2011, Moscow). Package comparison·booking B2C. Channels Russian outbound to Vietnam(1)·Thailand·Korea·Singapore → a new Russian channel for OMH Asia hotels. [Contracting entity] UAE company Vivatech Platform L.L.C-FZ (Dubai Meydan Free Zone, License 2643677.01, TRN 105369350100001, est. Jan 2026, director·100% sole shareholder Alexey Zaretsky·Israeli, Emirates Islamic Bank USD account). Direct API (Travelata builds), 5-10 bookings/day at start.'),
      check:B('★지분관계 확인 결과: UAE 계약법인 Vivatech은 Travelata(RU)와 자본상 무관 — 지분등록부상 개인 Alexey Zaretsky(이스라엘) 100% 단독 소유. 즉 OMH 계약 상대는 Travelata와 지분관계 없는 2026.1 설립 신설 프리존 법인이나, 별도 보증 대신 UAE 사업자등록증(Vivatech Trade License, License 2643677.01)을 계약·recourse 근거로 수용(관리자 방침) → recourse 해결. 법무/제재: 계약주체가 UAE 법인(Vivatech)이라 러시아 제재 대상 아님 → 통과(Emirates Islamic Bank USD 결제). 신설 법인·자본 AED 10만(~$27k) 얇음. 거래조건 합의 완료: Deposit $20k·신용 1:2(한도 $40k, 무담보 $20k)·격주(결제 3~5일). Deposit는 통합 착수 시 Pavel 송금 예정(현재 미수령).','★Ownership check: the UAE contracting entity Vivatech has NO capital link to Travelata (RU) — share register shows individual Alexey Zaretsky (Israeli) as 100% sole owner. So OMH would contract with a Jan-2026 new free-zone company unrelated to Travelata but instead of a separate guarantee, the UAE Trade License (Vivatech, License 2643677.01) is accepted as the contracting·recourse basis (management decision) → recourse resolved. Legal/sanctions: the contracting party is a UAE entity (Vivatech), not subject to Russia sanctions → cleared (Emirates Islamic Bank USD settlement). New entity, thin capital AED 100k (~$27k). Terms agreed: Deposit $20k·credit 1:2 (limit $40k, $20k uncovered)·biweekly (3-5 day). Deposit to be wired by Pavel closer to integration (not yet received).'),
      opinion:B('요청 서류(UAE Trade License·지분등록부·회사소개서)로 UAE 법인·은행·이사 확인(진전). 다만 Travelata와 지분관계 없는 개인 소유 신설 shell이라 recourse는 UAE 사업자등록증(Vivatech Trade License)을 근거로 수용(해결), 법무/제재 통과(계약주체 UAE), 거래조건($20k·1:2·한도 $40k·격주) 합의 완료 → 조건부 승인(B). 남은 절차: 소액 파일럿·초기 3개월 모니터링(계약서·정산합의서는 승인 후 컨펌).','Requested docs (UAE Trade License·share register·profile) confirm the UAE entity·bank·director (progress). But it is an individually-owned new shell with no ownership link to Travelata → recourse·guarantee is the key issue → stays needs-review (C). Legal/sanctions cleared (UAE contracting party). Preconditions: (1) confirm Alexey Zaretsky↔Travelata relationship·guarantee (recourse) — the key item, (2) commercial terms agreed (deposit $20k·1:2·$40k limit·biweekly), (3) small pilot.'),
      comment:B('recourse는 별도 보증 대신 UAE 사업자등록증(Vivatech Trade License)을 계약·recourse 근거로 수용(관리자 방침)해 해결. 법무/제재 통과·거래조건($20k·1:2·격주) 합의 완료 → 조건부 승인 가능(4/5). 소액 파일럿·초기 3개월 모니터링.','Recourse resolved: instead of a separate guarantee, the UAE Trade License (Vivatech) is accepted as the contracting·recourse basis (management decision). Legal/sanctions cleared and terms ($20k·1:2·biweekly) agreed → conditional approval OK (4/5). Small pilot·first 3-month monitoring.')},
    history:[{stage:'sales',reviewer:'Global Sales',decision:'hold',comment:B('신규 OTA — Travelata(RU, Travel Technology LLC 2011). 계약은 UAE 법인(OMH SG). 러시아 아웃바운드→베트남·태국·한국·싱가포르. Deposit $10k·신용 1:4($40k)·격주(결제 5일). ★러시아 제재 법무검토·UAE 법인 확인 필요.','New OTA — Travelata (RU, Travel Technology LLC 2011). Contract via UAE entity (OMH SG). Russia outbound→Vietnam·Thailand·Korea·Singapore. Deposit $10k·credit 1:4 ($40k)·biweekly (5-day). Requires Russia-sanctions legal review·UAE entity verification.'),date:'2026-07-17'},
      {stage:'scm',reviewer:'Global OPs',decision:'hold',comment:B('요청 서류 검토: UAE 계약법인 Vivatech Platform L.L.C-FZ(Meydan FZ, License 2643677.01, 2026.1, 이사·100% 주주 Alexey Zaretsky·이스라엘, Emirates Islamic USD). ★지분등록부상 Travelata(RU)와 지분관계 없음 → 보증·recourse 및 제재 법무검토 조건 유지.','Doc review: UAE entity Vivatech Platform L.L.C-FZ (Meydan FZ, License 2643677.01, est. 2026-01, director·100% shareholder Alexey Zaretsky·Israeli, Emirates Islamic USD). Share register shows NO ownership link to Travelata (RU) → guarantee·recourse and sanctions legal review remain conditions.'),date:'2026-07-17'},
      {stage:'finance',reviewer:'Global OPs',decision:'proceed',comment:B('법무/제재 검토 통과 — 계약주체가 UAE 법인(Vivatech)이라 러시아 제재 대상 아님. 남은 관문: Vivatech↔Travelata 보증(recourse)·신용/Deposit 조건.','Legal/sanctions cleared — contracting party is a UAE entity (Vivatech), not subject to Russia sanctions. Remaining: Vivatech↔Travelata guarantee (recourse)·credit/deposit terms.'),date:'2026-07-17'},
